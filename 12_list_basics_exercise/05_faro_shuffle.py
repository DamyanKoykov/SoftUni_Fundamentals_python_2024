...
cards_list = string_of_cards.split()  # converts string_of_cards to list
middle_of_the_cards = int(len(cards_list) / 2)
end_of_card_list = len(cards_list)
# split card_list in two even card lists
...
for current_shuffle in range(1, shuffle_count + 1):
    ...
    #  we split the deck in half again for the next shuffle
    first_half = result[0:middle_of_the_cards]
    second_half = result[middle_of_the_cards:]
    if current_shuffle != shuffle_count:  # if it is not the last shuffle we have to reset the card list
        result = []
print(result)
